...
/*
 * MMM-FaceWake's hand tracking (MediaPipe Hands) needs a WebGL2 context.
 * When hardware acceleration is disabled (the default above), Chromium blocks
 * WebGL, which makes MediaPipe abort. These switches allow a software
 * (SwiftShader) WebGL fallback so it can initialize even without a GPU.
 */
...
function createWindow () {
	...
	// Allow camera/microphone access (used by modules such as MMM-FaceWake).
	mainWindow.webContents.session.setPermissionRequestHandler((webContents, permission, callback) => {
		callback(permission === "media");
	});
	mainWindow.webContents.session.setPermissionCheckHandler((webContents, permission) => permission === "media");
